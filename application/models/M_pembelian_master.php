...
{
	function insert_master($nomor_nota, $tanggal, $id_kasir, $id_supplier, $bayar, $cara_bayar, $tempo, $grand_total, $catatan)
	{
		$dt = array(
			...
			'id_supplier' => (empty($id_supplier)) ? NULL : $id_supplier,
			'id_user' => $id_kasir
		);
	...
	function fetch_data_pembelian($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			SELECT 
				(@row:=@row+1) AS nomor, 
	...
	function get_baris($id_pembelian)
	{
		$sql = "
			SELECT 
				a.`nomor_nota`, 
	...
	{
		return $this->db->select('nomor_nota')->where('nomor_nota', $nota)->limit(1)->get('pj_pembelian_master');
	}
}