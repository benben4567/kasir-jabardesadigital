...
class M_kategori_barang extends CI_Model 
{
	...
	function fetch_data_kategori($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			...
			FROM 
				...
				LEFT JOIN `pj_subkategori_barang` AS b ON b.`kategorisub` = a.`kategori` , (SELECT @row := 0) r WHERE 1=1 
		AND a.`dihapus` = 'tidak'
		";
	...
	function tambah_sub_kategori($kategori,$subkategori,$subkategori2,$subkategori3)
	{
		$dt = array(
			'kategorisub' => $kategori,
			'subkategori' => $subkategori,
			'subkategori2' => $subkategori2,
			'subkategori3' => $subkategori3,
			'dihapus' => 'tidak'
		);

		return $this->db->insert('pj_subkategori_barang', $dt);
	}
