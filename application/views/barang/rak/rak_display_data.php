...
<div class="container">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5><i class='fa fa-cube fa-fw'></i> Barang <i class='fa fa-angle-right fa-fw'></i> Display Produk Pindah Rak</h5>
			<hr />
...
{
	$tambahan .= nbs(2)."<a href='".site_url('barang/tambah-display-rak')."' class='btn btn-default' id='TambahRak'><i class='fa fa-plus fa-fw'></i> Tambah Display Rak</a>";
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('rak/list-display-rak-json'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#HapusRak', function(e){
		e.preventDefault();
		...
		$('#ModalFooter').html("<button type='button' class='btn btn-primary' id='YesDeleteRak' data-url='"+Link+"'>Ya, saya yakin</button><button type='button' class='btn btn-default' data-dismiss='modal'>Batal</button>");
		$('#ModalGue').modal('show');
	});

	$(document).on('click', '#YesDeleteRak', function(e){
		e.preventDefault();
		$('#ModalGue').modal('hide');
	...
	$(document).on('click', '#TambahRak, #EditRak', function(e){
		e.preventDefault();


		$('.modal-dialog').addClass('modal-lg');
		if($(this).attr('id') == 'TambahRak')
		{
			$('#ModalHeader').html('Tambah Display Rak');
		}
		if($(this).attr('id') == 'EditRak')
		{
			$('#ModalHeader').html('Edit Rak');
		}
