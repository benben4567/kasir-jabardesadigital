...
<div class="container">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5><i class='fa fa-cube fa-fw'></i> Barang <i class='fa fa-angle-right fa-fw'></i> List Warna</h5>
			<hr />
...
{
	$tambahan .= nbs(2)."<a href='".site_url('barang/tambah-warna')."' class='btn btn-default' id='TambahWarna'><i class='fa fa-plus fa-fw'></i> Tambah Warna</a>";
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('barang/list-warna-json'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#HapusWarna', function(e){
		e.preventDefault();
		...
		$('#ModalFooter').html("<button type='button' class='btn btn-primary' id='YesDeleteWarna' data-url='"+Link+"'>Ya, saya yakin</button><button type='button' class='btn btn-default' data-dismiss='modal'>Batal</button>");
		$('#ModalGue').modal('show');
	});

	$(document).on('click', '#YesDeleteWarna', function(e){
		e.preventDefault();
		$('#ModalGue').modal('hide');
	...
	$(document).on('click', '#TambahWarna, #EditWarna', function(e){
		...
		{
			$('#ModalHeader').html('Tambah Warna');
		}
		if($(this).attr('id') == 'EditWarna')
		{
			$('#ModalHeader').html('Edit Warna');
		}
