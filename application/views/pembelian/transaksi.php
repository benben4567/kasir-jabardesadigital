			...
			<div class='row'>
				<div class='col-sm-3'>
					...
					<div class="panel panel-primary" id='PelangganArea'>
						<div class="panel-heading"><i class='fa fa-user'></i> Informasi Supplier</div>
						<div class="panel-body">
							<div class="form-group">
								<label>Supplier</label>
								<a href="<?php echo site_url('pembelian/tambah-supplier'); ?>" class='pull-right' id='TambahPelanggan'>Tambah Baru ?</a>
								<select name='id_supplier' id='id_supplier' class='form-control input-sm' style='cursor: pointer;'>
									<option value=''>-- Umum --</option>
									<?php
									if($supplier->num_rows() > 0)
									{
										foreach($supplier->result() as $p)
										{
											echo "<option value='".$p->id_supplier."'>".$p->nama."</option>";
										}
									}
									?>
								</select>
							...
							<div class="form-horizontal">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<div id='telp_supplier'><small><i>Tidak ada</i></small></div>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-4 control-label">Alamat</label>
									<div class="col-sm-8">
										<div id='alamat_supplier'><small><i>Tidak ada</i></small></div>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-4 control-label">Info Lain</label>
									<div class="col-sm-8">
										<div id='info_tambahan_supplier'><small><i>Tidak ada</i></small></div>
									</div>
								</div>
							</div>
				...
				<div class='col-sm-9'>
					<h5 class='judul-transaksi'>
						<i class='fa fa-shopping-cart fa-fw'></i> Pembelian <i class='fa fa-angle-right fa-fw'></i> Transaksi
						<a href="<?php echo site_url('pembelian/transaksi'); ?>" class='pull-right'><i class='fa fa-refresh fa-fw'></i> Refresh Halaman</a>
					</h5>
					...
					<div class='row'>
						<div class='col-sm-7'>
							...
							<div class='row'>
								...
								<div class='col-sm-6'>F8 = Fokus ke field ongkir</div>
						...
						<div class='col-sm-5'>
							<div class="form-horizontal">
								<div class="form-group">
									<label class="col-sm-6 control-label">Ongkos Kirim (F8)</label>
									<div class="col-sm-6">
										<input type='text' name='cash' id='UangCash' class='form-control' onkeypress='return check_int(event)'>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-6 control-label">Cara Pembayaran</label>
									<div class="col-sm-6">
										<select class="form-control" name="cara_bayar" id="cara_bayar" onchange="yesnoCheck(this);"><option value="cash">Cash</option><option value="hutang">Hutang</option></select>
									</div>
								</div>

									<div id="ifYes" style="display: none;">
												<div class="form-group">
										<label class="col-sm-6 control-label">Tempo</label>
										<div class="col-sm-6">
                              <input type="text" class='form-control' id="tempo" name="tempo" placeholder="tgl/bln/tahun" />
                                     </div></div>
									</div>
								<div class="form-group">
...
$(document).ready(function(){
	...
	$('#id_supplier').change(function(){
		...
		{
			$.ajax({
				url: "<?php echo site_url('pembelian/ajax-supplier'); ?>",
				type: "POST",
				cache: false,
				...
				success: function(json){
					...
					$('#info_tambahan_supplier').html(json.info_tambahan);
				}
			});
		}
		else
		{
			$('#telp_supplier').html('<small><i>Tidak ada</i></small>');
			$('#alamat_supplier').html('<small><i>Tidak ada</i></small>');
			$('#info_tambahan_supplier').html('<small><i>Tidak ada</i></small>');
		}
	});
...
function SimpanTransaksi()
{
    ...
    FormData += "&tempo="+encodeURI($('#tempo').val());
	$.ajax({
