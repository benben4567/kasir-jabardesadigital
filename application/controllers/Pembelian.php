...
class Pembelian extends MY_Controller 
{
	...
	public function transaksi()
	{
		...
		if($level == 'admin' OR $level == 'kasir')
		{
					...
					if($total > 0)
					{
						...
						if($this->form_validation->run() == TRUE)
						{
							...
							else
							{
								...
								$master = $this->m_pembelian_master->insert_master($nomor_nota, $tanggal, $id_kasir, $id_supplier, $bayar,$cara_bayar,$tempo, $grand_total, $catatan);
								if($master)
								{
									...
									foreach($_POST['kode_barang'] as $k)
									{
										if( ! empty($k))
										{
											...
											$insert_detail	= $this->m_pembelian_detail->insert_detail($id_master, $id_barang, $jumlah_beli, $harga_satuan, $sub_total);
											if($insert_detail)
											{
			...
			else
			{
				...
				$dt['supplier']= $this->m_supplier->get_all();
				$this->load->view('pembelian/transaksi', $dt);
			}
		}
	}
	...
	public function transaksi_cetak()
	{
		...
		{
			$supplier = $this->m_supplier->get_baris($id_supplier)->row()->nama;
		}
		...
		$pdf->Ln();
		$pdf->Ln();
		$pdf->Cell(130, 5, "Beli Sesuai Kebutuhan Toko", 0, 0, 'C');
	...
	public function ajax_supplier()
	{
		if($this->input->is_ajax_request())
		{
			...
			$data = $this->m_supplier->get_baris($id_supplier)->row();
	...
	public function supplier()
	{
		...
		if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan')
		{
			$this->load->view('pembelian/supplier_data');
		}
	}
	...
	public function supplier_json()
	{
		...
		$requestData	= $_REQUEST;
		$fetch			= $this->m_supplier->fetch_data_supplier($requestData['search']['value'], $requestData['order'][0]['column'], $requestData['order'][0]['dir'], $requestData['start'], $requestData['length']);
		...
		{ 
			...
			if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan') 
			{
				$nestedData[]	= "<a href='".site_url('pembelian/supplier-edit/'.$row['id_supplier'])."' id='Editsupplier'><i class='fa fa-pencil'></i> Edit</a>";
			}

			if($level == 'admin') 
			{
				$nestedData[]	= "<a href='".site_url('pembelian/supplier-hapus/'.$row['id_supplier'])."' id='Hapussupplier'><i class='fa fa-trash-o'></i> Hapus</a>";
			}
		...
		if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan')
		{
			if($_POST)
			{
				...
				if($this->form_validation->run() == TRUE)
				{
                    ...
                    $nmbisnis 		= $this->input->post('nmbisnis');
                    $nobisnis 		= $this->input->post('nobisnis');
					...
					$insert 	= $this->m_supplier->tambah_supplier($nama, $alamat, $telepon, $info, $nmbisnis, $nobisnis, $saldo, $kota, $nopajak, $alamatp, $pembayaran, $unique);
					if($insert)
					{
						$id_supplier = $this->m_supplier->get_dari_kode($unique)->row()->id_supplier;
						echo json_encode(array(
							'status' => 1,
							'pesan' => "<div class='alert alert-success'><i class='fa fa-check'></i> <b>".$nama."</b> berhasil ditambahkan sebagai supplier.</div>",
			...
			{
				$this->load->view('pembelian/supplier_tambah');
			}
		}
	}

	public function supplier_edit($id_supplier = NULL)
	{
		if( ! empty($id_supplier))
		{
			$level = $this->session->userdata('ap_level');
			if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan')
			{
				if($this->input->is_ajax_request())
				{
					...
					$unique		= time().$this->session->userdata('ap_id_user');
							...
							$update 	= $this->m_supplier->update_supplier($nama, $alamat, $telepon, $info, $nmbisnis, $nobisnis, $saldo, $kota, $nopajak, $alamatp, $pembayaran, $unique);
							if($update)
							{
								echo json_encode(array(
									'status' => 1,
					...
					{
						$dt['supplier'] = $this->m_supplier->get_baris($id_supplier)->row();
						$this->load->view('pembelian/supplier_edit', $dt);
					}
				}
			}
		}
	...
	public function supplier_hapus($id_supplier)
	{
		$level = $this->session->userdata('ap_level');
		if($level == 'admin')
		{
			...
			{
				$this->load->model('m_supplier');
				$hapus = $this->m_supplier->hapus_supplier($id_supplier);
				if($hapus)
				{
					echo json_encode(array(
