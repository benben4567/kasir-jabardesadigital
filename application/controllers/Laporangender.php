...
class Laporangender extends MY_Controller 
{
	...
	public function index()
	{
     ...
     $data['rak'] = $rak;
      $data['merek'] = $merek;
		$this->load->view('laporan/form_laporan_gender', $data);
	}

	public function penjualan($from, $to, $rak, $gender, $warna)
	{
		$this->load->model('m_penjualan_master');
		$dt['penjualan'] 	= $this->m_penjualan_master->laporan_penjualan_gender($from, $to, $rak, $gender, $warna);
		$dt['from']			= date('d F Y', strtotime($from));
		$dt['to']			= date('d F Y', strtotime($to));
		$dt['rak']			= $rak;
		$dt['gender']			= $gender;
		$dt['warna']			= $warna;
		$this->load->view('laporan/laporan_penjualan_gender', $dt);
	}

	public function excel($from, $to, $rak, $gender,$warna)
	{
		$this->load->model('m_penjualan_master');
		$penjualan 	= $this->m_penjualan_master->laporan_penjualan_rak($from, $to, $rak, $gender, $warna);
		if($penjualan->num_rows() > 0)
		{
			...
			echo "
				<h4>Laporan Penjualan by Gender Tanggal ".date('d/m/Y', strtotime($from))." - ".date('d/m/Y', strtotime($to))."</h4>
	...
	public function pdf($from, $to, $rak, $gender, $warna)
	{
		...
		$pdf->SetFont('Arial','',9);
		$pdf->Cell(0, 8, "Laporan Penjualan by Gender Tanggal ".date('d/m/Y', strtotime($from))." - ".date('d/m/Y', strtotime($to)), 0, 1, 'L'); 
		...
		$pdf->Cell(20, 7, 'Warna', 1, 0, 'L'); 
		//$pdf->Cell(35, 7, 'Nota', 1, 0, 'L'); 
		$pdf->Cell(30, 7, 'Total Penjualan', 1, 0, 'L'); 
		$pdf->Ln();

		$this->load->model('m_penjualan_master');
		$penjualan 	= $this->m_penjualan_master->laporan_penjualan_gender($from, $to, $rak, $gender, $warna);
